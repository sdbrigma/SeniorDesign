...
void InitAdc(void)
{
    ...
    AdcRegs.ADCCTL1.bit.ADCREFSEL = 1;      // Select interal BG.  Set to 1 to use external reference
    EDIS;
...
void ConfigADC(void){
	...
	AdcRegs.SOCPRICTL.bit.SOCPRIORITY = 0x01; // SOC0 is high priority SOC1-15 are in round robin

    AdcChanSelect(1); // Select ADCINA1 as the channel for SOC0-15
